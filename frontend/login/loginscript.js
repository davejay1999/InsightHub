...
signUpForm.addEventListener('submit', function (event)
{
	...
	function handleSignup(event)
	{
		...
		fetch("http://3.90.66.68:3000/user/management/register",
			{
				method: "POST",
				headers:
				{
			...
			.then(data =>
			{
				...
				else
				{
					...
					const signUpSuccessfulDiv = document.querySelector('.sign-up-successful');
					signUpSuccessfulDiv.innerHTML = "Sign up successful!, Please Sign In!"; // Set inner HTML content
					container.classList.remove("sign-up-mode");
				}
			})
			.catch(error =>
	...
	if (!validateEmail(email) || !validatePassword(password))
	{
		...
		else if (!validatePassword(password))
		{
			...
			displayPopup(".password-container", "Password must be 8 characters long, include 1 uppercase, 1 lowercase, 1 number, & 1 special character.");
		}
		else
		{
			console.log("all field container");
			displayPopup(".all-field-container", "Please fill in all fields for signing up.");
		}
	}
	else
	{
...
function handleSignin(event)
{
	...
	fetch("http://3.90.66.68:3000/user/management/login",
		...
		.catch(error =>
		{
			...
			// Handle errors here
			// Display error message in a pop-up
			displayPopup(".sign-in-error", "An error occurred. Please try again later.");
		});
}
